...
class FieldDataset(Dataset):
    ...
    def __init__(
        ...
        tgt_patterns,
        style_pattern=None,
        noise_patterns=None,  # normalized noise patterns, same size as the input field
        noise_style_pattern=None,  # noise style pattern
        in_norms=None,
        tgt_norms=None,
        noise_norms=None,
        callback_at=None,
        ...
        **kwargs,
        ...
        print("num_snapshots", num_snapshots, "sample_snaps", sample_snaps)
        sample_idx = list(
            WeightedRandomSampler(
                weights=torch.ones(num_snapshots),
                num_samples=min(sample_snaps, num_snapshots),
                replacement=False,
            )
        )
        ...
        # load noise fields
        self.has_noise_pattern = noise_patterns is not None
        if noise_patterns is not None:
            noise_lists = [sorted(glob(p)) for p in noise_patterns]
            if sampling:
                sampled_noise_list = [[x[i] for i in sample_idx] for x in noise_lists]
                self.noise_files = list(zip(*sampled_noise_list))
            else:
                self.noise_files = list(zip(*noise_lists))

        # ------------------------ end loading data ------------------------
        if len(self.in_files) != len(self.tgt_files) != len(self.noise_files):
        ...
        if noise_patterns is not None:
            self.noise_chan = [
                np.load(f, mmap_mode="r").shape[0] for f in self.noise_files[0]
            ]
        ...
        self.noise_style = noise_style_pattern is not None
        self.noise_style_size = 0
        if self.noise_style:
            noise_style_files = sorted(glob(noise_style_pattern))
            if sampling:
                sampled_noise_style_list = [noise_style_files[i] for i in sample_idx]
                self.noise_style_files = sampled_noise_style_list
            else:
                self.noise_style_files = noise_style_files
            self.noise_style_size = np.load(self.noise_style_files[0]).shape[0]
        ...
        if noise_norms is not None and len(noise_patterns) != len(noise_norms):
            raise ValueError(
                "numbers of noise normalization functions and fields do not match"
            )
        self.noise_norms = noise_norms
        ...
        if noise_pad is not None and noise_patterns is not None:
    ...
    def __getitem__(self, idx):
        ...
        in_fields = [np.load(f, mmap_mode=mmap_mode) for f in self.in_files[ifile]]
        tgt_fields = [np.load(f, mmap_mode=mmap_mode) for f in self.tgt_files[ifile]]
        if self.has_noise_pattern:
            noise_fields = [
                np.load(f, mmap_mode=mmap_mode) for f in self.noise_files[ifile]
            ]
        ...
        in_fields = [torch.from_numpy(f.astype(np.float32)) for f in in_fields]
        tgt_fields = [torch.from_numpy(f.astype(np.float32)) for f in tgt_fields]
        if self.has_noise_pattern:
            noise_fields = [
                torch.from_numpy(f.astype(np.float32)) for f in noise_fields
            ]
        ...
        noise_style = torch.empty(0, dtype=torch.float32)
        if self.noise_style:
            noise_style = np.load(self.noise_style_files[ifile])
            noise_style = torch.from_numpy(noise_style.astype(np.float32))
        ...
        if self.tgt_norms is not None:
            for norm, x in zip(self.tgt_norms, tgt_fields):
                norm = import_attr(norm, norms, callback_at=self.callback_at)
                norm(x, a=style, **self.kwargs)
        if self.noise_norms is not None:
            for norm, x in zip(self.noise_norms, noise_fields):
                norm = import_attr(norm, norms, callback_at=self.callback_at)
                norm(x, a=noise_style, **self.kwargs)
        ...
        if self.augment:
            ...
            if self.has_noise_pattern:
                flip_axes = flip(noise_fields, flip_axes, self.ndim)

            perm_axes = perm(in_fields, perm_axes, self.ndim)
            perm_axes = perm(tgt_fields, perm_axes, self.ndim)

            if self.has_noise_pattern:
                perm_axes = perm(noise_fields, perm_axes, self.ndim)
        ...
        if self.aug_add is not None:
            ...
            if self.has_noise_pattern:
                add_fac = add(noise_fields, add_fac, self.aug_add)
        ...
        if self.aug_mul is not None:
            ...
            if self.has_noise_pattern:
                mul_fac = mul(noise_fields, mul_fac, self.aug_mul)

        in_fields = torch.cat(in_fields, dim=0)
        tgt_fields = torch.cat(tgt_fields, dim=0)

        if self.has_noise_pattern:
            noise_fields = torch.cat(noise_fields, dim=0)
        ...
        return {
            ...
            "target": tgt_fields,
            "style": style,
            "noise": noise_fields if self.has_noise_pattern else 0,
            "noise_style": noise_style if self.noise_style else 0,
    ...
    def assemble(self, label, chan, patches, paths):
        ...
        if patches.shape[1] != sum(chan):
            raise RuntimeError(
                "number of channels mismatch: " f"{patches.shape[1], sum(chan)}"
            )
        if len(paths) != len(chan):
            raise RuntimeError("number of fields mismatch: " f"{len(paths), len(chan)}")
        paths = list(zip(*paths))
        if patches.shape[0] != len(paths):
            raise RuntimeError(
                "batch size mismatch: " f"{patches.shape[0], len(paths)}"
            )
...
def fill(field, patch, anchor):
    ...
    if not field.ndim == patch.ndim == 1 + ndim:
        raise RuntimeError("ndim mismatch: " f"{field.ndim, patch.ndim, 1 + ndim}")
...
def crop(fields, anchor, crop, pad):
    ...
    if not ndim == len(anchor) == len(crop) == len(pad):
        raise RuntimeError(
            "ndim mismatch: " f"{ndim, len(anchor), len(crop), len(pad)}"
        )
