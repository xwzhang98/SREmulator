...
class DistFieldSampler(Sampler):
    ...
    def __init__(
        ...
        print(f"nsample: {self.nsample}, nfile: {self.nfile}, ncrop: {self.ncrop}")
        self.sampling = sampling
        if self.sampling:
            self.num_sampled = self.nsample // 4
    ...
    def __len__(self):
        if self.sampling:
            return self.num_sampled // self.world_size
        else:
