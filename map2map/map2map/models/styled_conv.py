...
class ConvStyledBlock(nn.Module):
    ...
    def _get_layer(self, l):
        if l == "U":
            ...
            return ConvStyled3d(
                in_chan, out_chan, self.style_size, 2, stride=2, resample="U"
            )
        elif l == "D":
            in_chan, out_chan = self._setup_conv()
            return ConvStyled3d(
                in_chan, out_chan, self.style_size, 2, stride=2, resample="D"
            )
        elif l == "C":
            in_chan, out_chan = self._setup_conv()
            return ConvStyled3d(
                in_chan, out_chan, self.style_size, self.kernel_size, stride=self.stride
            )
        elif l == "B":
